...
export const createVisit = async (req, res) => {
    try {
        ...
        const [area] = await db
            .select()
            .from(areasTable)
            .where(and(eq(areasTable.id, areaId), eq(areasTable.companyId, companyId)));
        if (!area) {
        ...
        const [customer] = await db
            .select()
            .from(customersTable)
            .where(and(eq(customersTable.id, customerId), eq(customersTable.companyId, companyId), eq(customersTable.areaId, areaId)));
        if (!customer) {
            return res.status(400).json({
        ...
        const [activeVisit] = await db
            .select()
            .from(visitsTable)
            .where(and(eq(visitsTable.salesmanId, user.userId), isNull(visitsTable.checkOutAt)));
        if (activeVisit) {
            return res.status(400).json({
    ...
    try {
        ...
        const data = await db.transaction(async (tx) => {
            ...
            const [visit] = await tx
                .select()
                .from(visitsTable)
                .where(and(eq(visitsTable.id, id), eq(visitsTable.salesmanId, user.userId)));
            if (!visit) {
            ...
            if (result === "new order") {
                ...
                if (transactionType === "cash") {
                    ...
                    else if (paid < totals.finalAmount)
                ...
                const [newTransaction] = await tx
                    ...
                    remainingAmount: (totals.finalAmount - finalPaidAmount).toString(),
                })
                    .returning();
                if (!newTransaction) {
                ...
                await tx.insert(transactionItemsTable).values(products.map((p) => {
                    const price = Number(p.price);
                    const quantity = Number(p.quantity);
                    const discount = Number(p.discount || 0);
                    const subtotal = price * quantity;
...
export const getAllVisits = async (req, res) => {
    try {
        ...
        /* ================= DATE FILTER ================= */
        if (startDate) {
            conditions.push(gte(visitsTable.checkInAt, new Date(startDate)));
        }
        if (endDate) {
            conditions.push(lte(visitsTable.checkInAt, new Date(endDate)));
        }
        /* ================= QUERY ================= */
...
export const getVisitById = async (req, res) => {
    try {
        ...
        const visit = await db
            ...
            .leftJoin(customersTable, eq(visitsTable.customerId, customersTable.id))
            .leftJoin(areasTable, eq(visitsTable.areaId, areasTable.id))
            .where(and(eq(visitsTable.id, id), eq(visitsTable.companyId, user.companyId)))
            .limit(1);
        if (!visit.length) {
        ...
        const transactions = await db
            ...
            .where(and(eq(transactionsTable.visitId, id), eq(transactionsTable.companyId, user.companyId)));
        ...
        const formattedTransactions = transactions.map((t) => ({
            ...t,
            items: transactionItems.filter((item) => item.transactionId === t.transactionId),
        }));
        return res.status(200).json({
