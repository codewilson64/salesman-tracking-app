...
export const createCustomer = async (req, res) => {
    try {
        const { areaId, customerName, shopName, phone, address, description, customerImage, customerImageId, latitude, longitude } = req.body;
        const user = req.user;
        if (!user?.companyId) {
...
export const getAllCustomers = async (req, res) => {
    try {
        ...
        if (user.role === "salesman") {
            ...
            condition = and(eq(customersTable.companyId, user.companyId), eq(areasTable.salesmanId, salesman.id) // ✅ filter via area
            );
        }
        else {
...
export const getCustomerById = async (req, res) => {
    try {
        ...
        const [customer] = await db
            ...
            .leftJoin(areasTable, eq(customersTable.areaId, areasTable.id))
            .leftJoin(salesmenTable, eq(areasTable.salesmanId, salesmenTable.id))
            .where(and(eq(customersTable.id, id), eq(customersTable.companyId, user.companyId)));
        if (!customer) {
    ...
    try {
        ...
        if (areaId) {
            const [area] = await db
                .select()
                .from(areasTable)
                .where(and(eq(areasTable.id, areaId), eq(areasTable.companyId, user.companyId)));
            if (!area) {
                return res.status(400).json({
