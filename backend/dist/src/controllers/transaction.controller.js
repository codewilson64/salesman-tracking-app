...
export const getOutstandingTransactions = async (req, res) => {
    try {
        const user = req.user;
        ...
        conditions.push(or(eq(transactionsTable.paymentStatus, "unpaid"), eq(transactionsTable.paymentStatus, "partial")));
        /* ================= DATE FILTER ================= */
        if (startDate) {
            conditions.push(gte(transactionsTable.createdAt, new Date(startDate)));
        }
        if (endDate) {
            conditions.push(lte(transactionsTable.createdAt, new Date(endDate)));
        }
        /* ================= QUERY ================= */
...
export const getTransactionById = async (req, res) => {
    try {
        ...
        const [transaction] = await db
            ...
            .leftJoin(visitsTable, eq(transactionsTable.visitId, visitsTable.id))
            .leftJoin(customersTable, eq(visitsTable.customerId, customersTable.id))
            .where(and(eq(transactionsTable.id, id), eq(transactionsTable.companyId, user.companyId)));
        if (!transaction) {
